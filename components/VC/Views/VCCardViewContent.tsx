...
export const VCCardViewContent: React.FC<VCItemContentProps> = ({
  ...
  // Filter disclosed keys to only show fields requested by verifier (per OpenID4VP spec)
  const allowedDisclosedKeys = React.useMemo(() => {
    if (!credential?.disclosedKeys) {
      return [];
    }

    // If no requestedClaims, show all (non-VP flow or no filtering needed)
    if (!requestedClaims) {
      return credential.disclosedKeys;
    }

    const requestedSet = new Set(
      requestedClaims.split(',').map(claim => claim.trim()),
    );

    console.log(`\n🔒 [SELECTIVE DISCLOSURE] Filtering disclosed keys:`);
    console.log(
      `   - All disclosed keys: ${credential.disclosedKeys.join(', ')}`,
    );
    console.log(`   - Requested by verifier: ${requestedClaims}`);

    const filtered = credential.disclosedKeys.filter(key => {
      const isAllowed = requestedSet.has(key);
      if (!isAllowed) {
        console.log(`   ⛔ Filtering out unrequested field: ${key}`);
      }
      return isAllowed;
    });

    console.log(`   ✅ Allowed fields: ${filtered.join(', ')}`);
    return filtered;
  }, [credential?.disclosedKeys, requestedClaims]);
  ...
  const areAllSelected = (): boolean => {
    return allowedDisclosedKeys.every(key => selectedFields[key]);
  };
  ...
  const toggleSelectAll = () => {
    ...
    if (areAllSelected()) {
      allowedDisclosedKeys.forEach(key => {
        updated[key] = false;
      });
    } else {
      allowedDisclosedKeys.forEach(key => {
      ...
      <Column>
        <Row
          ...
          <Row crossAlign="center">
            ...
            {(() => {
              const label = formatKeyLabel(name);
              const textColor = wellknownDisplayProperty.getTextColor(
                Theme.Colors.plainText,
              );
              console.log(
                `🏷️ [DisclosureNode] name="${name}" → label="${label}" color="${textColor}"`,
              );
              return (
                <Text
                  weight="semibold"
        ...
        <Row crossAlign="center" padding="3 0 0 3">
          ...
          <Column fill align={'space-around'} margin="0 10 0 10">
            <VCItemFieldValue
              ...
              fieldValue={(() => {
                console.log(
                  `\n🏷️ [VCCardViewContent] Getting credential type:`,
                );
                console.log(`   - Has wellknown: ${!!wellknown}`);
                console.log(`   - Has credential: ${!!credential}`);
                console.log(
                  `   - Credential has vct: ${!!credential?.fullResolvedPayload
                    ?.vct}`,
                );
                if (credential?.fullResolvedPayload?.vct) {
                  console.log(
                    `   - vct value: ${credential.fullResolvedPayload.vct}`,
                  );
                }
                return getCredentialType(wellknown, credential);
              })()}
        ...
        {flow === VCItemContainerFlowType.VP_SHARE &&
          ...
          credential?.disclosedKeys?.length > 0 && (
            <Column padding="8 0">
              ...
              {(() => {
                console.log(`\n📋 [Selective Disclosure] Building field tree:`);
                console.log(
                  `   - Has allowedDisclosedKeys: ${!!allowedDisclosedKeys}`,
                );
                console.log(
                  `   - allowedDisclosedKeys length: ${
                    allowedDisclosedKeys?.length || 0
                  }`,
                );
                console.log(`   - allowedDisclosedKeys:`, allowedDisclosedKeys);

                const tree = buildDisclosureTree(allowedDisclosedKeys);
                console.log(`   - Built tree keys:`, Object.keys(tree));
...
export interface VCItemContentProps {
  ...
  requestedClaims?: string; // Comma-separated claims requested by verifier
}
